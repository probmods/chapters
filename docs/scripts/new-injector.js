...
var app = {
  ...
  chapterName:
    typeof chapterName !== "undefined"
      ? chapterName
      : location.href.split("/").pop().replace(".html", "").split("#")[0]
};
      ...
      $.ajax({
        ...
        success: function(json) {
          ...
          if (json.code !== defaultCode) {
            $(ed.display.wrapper)
              .find(".code-settings")
              .prepend($("<span>(modified)</span>"));
          }
        },
        error: function() {
          console.log(
            "failure loading exercise " +
              ed.get("exerciseName") +
              ", using default"
          );
        },
    ...
    ed.on("run.finish", function() {
      setTimeout(function() {
        ...
        $svgs.map(function(i, svg) {
          ...
          $(svg).replaceWith(
            $("<img>").attr({
              src: "data:image/svg+xml;utf8," + svgHeader + svgText + "</svg>"
            })
          );
        });
      }, 0);
