...
// to run faster
/*  var iter = 0;
  var marblePos = marble.GetBody().GetPosition().y;
  while (bodyDef.position.y < canvas.height - marbleRadius && iter < 1000) {
    marblePos = marble.GetBody().GetPosition().y;
    world.Step(1/20, 8, 3);
    iter += 1;
  }
  world.DrawDebugData();
