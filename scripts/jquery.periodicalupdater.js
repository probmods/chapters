    ...
    $.PeriodicalUpdater = function(url, options, callback){
      var settings = jQuery.extend(true, {
        ...
        function getdata() {
          ...
          pu_log("preparing data for sending");
          pu_log(toSend);
          if(maxCalls == 0) {
            $.ajax(toSend);
      ...
      ajaxSettings.complete = function(data) {};
      ...
      ajaxSettings.success = function(data) {
        ...
        if (data == 'STOP_AJAX_CALLS') {
          maxCalls = -1;
          return;
        };
        var newData = jQuery.extend({}, data);
        delete newData["callback"];
        if(prevData == newData) {
          if(autoStop > 0) {
            noChange++;
          ...
          prevData        = newData;
          if(remoteData == null) remoteData = newData;
          if(settings.success) { settings.success(remoteData, true); }
          if(callback) callback(remoteData, true);
        }
